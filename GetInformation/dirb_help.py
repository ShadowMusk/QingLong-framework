...
def dirb_help():
    ...
    title = "Ways to use dirb"
    print(f"{title:^{table_width}}")
    # print(table)  # 把表格输出
    print(table_string)
    print("======================== EXAMPLES =======================")
    print("dirb http://url/directory/ (Simple Test)")
    print("dirb http://url/ -X .html (Test files with '.html' extension)")
    print("dirb http://url/ /usr/share/dirb/wordlists/vulns/apache.txt (Test with apache.txt wordlist)")
    print("dirb https://secure_url/ (Simple Test with SSL)")
