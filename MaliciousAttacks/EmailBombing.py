...
class EmailBombing:
        ...
        while True:
            try:
                ...
                if choice == 'back':
                    ...
                    self.thread_bomb(param_list[1], param_list[2], param_list[3], param_list[4], param_list[5], param_list[6], param_list[7], param_list[8])
                    continue
            except:
                print('\033[1;31;31m' + "[-] Execution failed.Please check your command and try again." + '\033[0m')
                continue

    def send_mail_bomb(self, server_mail_addr, port, from_addr, password, to_addr, template_path, subject):
        try:
    ...
    def thread_bomb(self, server_mail_addr, port, from_addr, password, to_addr, template_path, subject, thread_num):
        ...
        for i in range(int(thread_num)):
            t1 = threading.Thread(target=self.send_mail_bomb, args=(server_mail_addr, port, from_addr, password, to_addr, template_path, subject))
            t1.start()
            t1.join()
    ...
    def show_functions(self):
        ...
        mydata = [["1", "custom template", "1 server_mail_addr port from_addr password to_addr template_path subject thread_num", "使用自定义钓鱼邮件.模板格式为html"]]
    ...
    def show_params(self):
        ...
        mydata = [["server_mail_addr", "邮件服务器地址"], ["port", "邮件服务器端口"], ["from_addr", "发送方的邮箱"], ["password", "授权码"],
                  ["to_addr", "接收方邮箱"], ["template_path", "模板的绝对路径"], ["subject", "邮件标题"], ["thread_num", "同一时间发送邮件的数目"]]
        MyTable.createTable(headers, mydata)
